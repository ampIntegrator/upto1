 ...
 * images, the first one wide (16:8) when asked and the count is odd (so the others pair up, never
 * a lone half-width image), 12 px gaps, a caption below in mono. One column below 480 px of width.
 */
...
export function Gallery({images, wideFirst = true, caption}: GalleryProps) {
  ...
  const wide = wideFirst && images.length % 2 === 1;
  return (
    <VStack as="figure" gap={3} className={styles.root}>
      <VStack className={styles.grid}>
        {images.map((img, i) => (
          <VStack key={i} className={styles.cell} data-wide={(wide && i === 0) || undefined}>
