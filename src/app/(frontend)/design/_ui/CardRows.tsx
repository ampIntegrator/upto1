  ...
  return (
    <>
      <Section title="Bloc · image" note="Image pleine largeur de 230 px, titre centré avec son ornement, texte.">
        <Grid columns={{minWidth: 220, max: 4}} gap={4}>
          {[['bloc-a', 'Chiffrage instantané'], ['bloc-b', 'Suivi de chantier'], ['bloc-c', 'Devis client'], ['bloc-d', 'Maintenance multitechnique']].map(([s, t]) => (
            <OrbitaCard key={s} media={{type: 'image', src: IMG(s + seed)}} title={t} text={LOREM} cta={cta(linked, 'Découvrir')} />
          ))}
        </Grid>
      </Section>
      ...
      <Section title="Bloc · icône" note="Icône Nucleo de 28 px dans son carré de 64 px, couleur silo.">
        <Grid columns={{minWidth: 220, max: 4}} gap={4}>
          {([['clipboard-check', 'Chiffrage instantané'], ['gauge', 'Suivi des coûts'], ['table', 'Devis structurés'], ['shield', 'Données sécurisées']] as const).map(([k, t]) => (
            <OrbitaCard key={k} media={{type: 'icon', iconKey: k}} title={t} text={LOREM} cta={cta(linked, 'Découvrir')} />
          ))}
        </Grid>
      </Section>
      ...
      <Section title="Bloc · nombre" note="Grand nombre en Schibsted 800 couleur silo, préfixe et suffixe en highlight, espacés.">
        <Grid columns={{minWidth: 220, max: 4}} gap={4}>
          <OrbitaCard media={{type: 'number', value: '1 000', suffix: 'm²'}} title="Seuil d'application" text={LOREM} cta={cta(linked, 'En savoir plus')} />
          <OrbitaCard media={{type: 'number', value: '40', prefix: '−', suffix: '%'}} title="Objectif 2030" text={LOREM} cta={cta(linked, 'En savoir plus')} />
          <OrbitaCard media={{type: 'number', value: '30/09'}} title="Date limite annuelle" text={LOREM} cta={cta(linked, 'En savoir plus')} />
          <OrbitaCard media={{type: 'number', value: '850', prefix: '+'}} title="Courtiers équipés" text={LOREM} cta={cta(linked, 'En savoir plus')} />
        </Grid>
      </Section>
      ...
      <Section title="Bloc · titre seul" note="Sans média : le titre passe en couleur silo.">
        <Grid columns={{minWidth: 220, max: 4}} gap={4}>
          {['Architectes', "Maîtres d'œuvre", 'Promoteurs', 'Bailleurs sociaux'].map((t) => (
            <OrbitaCard key={t} media={{type: 'none'}} accentTitle title={t} text={LOREM} cta={cta(linked, 'Découvrir')} />
          ))}
        </Grid>
      </Section>
    </>
    ...
    <>
      <Section title="Article" note="Chip de catégorie, date, titre limité à deux lignes. Chip pleine (cat) sur la troisième.">
        <Grid columns={{minWidth: 240, max: 3}} gap={4}>
          <OrbitaCard preset="article" media={{type: 'image', src: IMG('post1' + seed)}} chip={{label: 'Chiffrage'}} date="12 sept. 2026" title="Du devis à la facturation : industrialiser le cycle commercial" cta={{label: 'Voir plus', href: '#'}} />
          <OrbitaCard preset="article" media={{type: 'image', src: IMG('post2' + seed)}} chip={{label: 'Chantier'}} date="4 sept. 2026" title="Suivre un chantier multi-sites sans perdre le fil" cta={{label: 'Voir plus', href: '#'}} />
          <OrbitaCard preset="article" media={{type: 'image', src: IMG('post3' + seed)}} chip={{label: 'Méthode', tone: 'cat'}} date="28 août 2026" title="Trois erreurs de chiffrage qui coûtent cher aux promoteurs" cta={{label: 'Voir plus', href: '#'}} />
        </Grid>
      </Section>

      <Section title="Réalisation" note="Chip accent, résultat chiffré, client et ville.">
        <Grid columns={{minWidth: 240, max: 3}} gap={4}>
          <OrbitaCard preset="realisation" media={{type: 'image', src: IMG('work1' + seed)}} chip={{label: 'Réhabilitation', tone: 'accent'}} result="+34 % closing" title="14 logements, rue des Lilas" client={{name: 'Habitat Rhône', location: 'Lyon'}} cta={{label: "Voir l'étude", href: '#'}} />
          <OrbitaCard preset="realisation" media={{type: 'image', src: IMG('work2' + seed)}} chip={{label: 'Tertiaire', tone: 'accent'}} result="−20 min / devis" title="Siège régional, 3 200 m²" client={{name: 'Groupe Alma', location: 'Nantes'}} cta={{label: "Voir l'étude", href: '#'}} />
          <OrbitaCard preset="realisation" media={{type: 'image', src: IMG('work3' + seed)}} chip={{label: 'Industriel', tone: 'accent'}} result="48 h de délai" title="Extension d'atelier, 900 m²" client={{name: 'Mécanique Vallée', location: 'Grenoble'}} cta={{label: "Voir l'étude", href: '#'}} />
        </Grid>
      </Section>
    </>
  );
