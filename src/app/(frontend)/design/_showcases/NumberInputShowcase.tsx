/* Showcase habillé Orbita — remplace la démo Astryx d'origine. Composant : src/components/NumberField */
'use client';
  ...
  return (
    <VStack gap={4}>
      ...
      <Grid columns={{minWidth: 240, max: 2}} gap={6} maxWidth={824}>
        <NumberField label="Quantité" value={qte} onChange={setQte} formatValue={(n) => `${n} lot${n > 1 ? 's' : ''}`} />
        <NumberField label="Surface (m²)" value={surface} onChange={setSurface} step={10} />
      </Grid>
    </VStack>
  );
}
