...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`case_studies_sheet_results\` (
  ...
  `)
  await db.run(sql`CREATE INDEX \`case_studies_sheet_results_order_idx\` ON \`case_studies_sheet_results\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`case_studies_sheet_results_parent_id_idx\` ON \`case_studies_sheet_results\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`case_studies_sheet_results_locales\` (
  	\`value\` text NOT NULL,
  	\`label\` text NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`_locale\` text NOT NULL,
  	\`_parent_id\` text NOT NULL,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`case_studies_sheet_results\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`case_studies_sheet_results_locales_locale_parent_id_unique\` ON \`case_studies_sheet_results_locales\` (\`_locale\`,\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`case_studies\` (
  	...
  	FOREIGN KEY (\`category_id\`) REFERENCES \`case_categories\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`case_studies_cover_idx\` ON \`case_studies\` (\`cover_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`case_studies_locales\` (
  	\`title\` text NOT NULL,
  	\`excerpt\` text,
  	\`content\` text,
  	\`sheet_location\` text,
  	\`sheet_location_label\` text,
  	\`sheet_deployment\` text,
  	\`sheet_deployment_label\` text,
  	\`sheet_modules\` text,
  	\`sheet_modules_label\` text,
  	\`sheet_card_result\` text,
  	\`sheet_cta_label\` text,
  	\`meta_title\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`case_studies_meta_meta_image_idx\` ON \`case_studies_locales\` (\`meta_image_id\`,\`_locale\`);`)
  await db.run(sql`CREATE UNIQUE INDEX \`case_studies_locales_locale_parent_id_unique\` ON \`case_studies_locales\` (\`_locale\`,\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`case_categories\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`slug\` text NOT NULL,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`case_categories_slug_idx\` ON \`case_categories\` (\`slug\`);`)
  await db.run(sql`CREATE INDEX \`case_categories_updated_at_idx\` ON \`case_categories\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`case_categories_created_at_idx\` ON \`case_categories\` (\`created_at\`);`)
  await db.run(sql`CREATE TABLE \`case_categories_locales\` (
  	\`title\` text NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`_locale\` text NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`case_categories\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`case_categories_locales_locale_parent_id_unique\` ON \`case_categories_locales\` (\`_locale\`,\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`portfolio_locales\` (
  ...
  await db.run(sql`ALTER TABLE \`payload_locked_documents_rels\` ADD \`case_categories_id\` integer REFERENCES case_categories(id);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`case_studies_sheet_results\`;`)
  await db.run(sql`DROP TABLE \`case_studies_sheet_results_locales\`;`)
  ...
  await db.run(sql`DROP TABLE \`case_categories\`;`)
  await db.run(sql`DROP TABLE \`case_categories_locales\`;`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "pages_id", "sections_id", "posts_id", "categories_id", "authors_id", "media_id", "users_id") SELECT "id", "order", "parent_id", "path", "pages_id", "sections_id", "posts_id", "categories_id", "authors_id", "media_id", "users_id" FROM \`payload_locked_documents_rels\`;`)
