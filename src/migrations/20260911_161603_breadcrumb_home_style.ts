...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_pages\`("id", "slug", "silo", "hero_variant", "hero_primary_href", "hero_primary_icon_key", "hero_secondary_href", "hero_secondary_icon_key", "hero_image_id", "hero_video_id", "hero_poster_id", "hero_overlay", "hero_media_id", "hero_breadcrumb", "hero_breadcrumb_mode", "updated_at", "created_at") SELECT "id", "slug", "silo", "hero_variant", "hero_primary_href", "hero_primary_icon_key", "hero_secondary_href", "hero_secondary_icon_key", "hero_image_id", "hero_video_id", "hero_poster_id", "hero_overlay", "hero_media_id", "hero_breadcrumb", "hero_breadcrumb_mode", "updated_at", "created_at" FROM \`pages\`;`)
  ...
  await db.run(sql`ALTER TABLE \`settings\` ADD \`breadcrumb_home_style\` text DEFAULT 'icon';`)
}
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`CREATE INDEX \`pages_updated_at_idx\` ON \`pages\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`pages_created_at_idx\` ON \`pages\` (\`created_at\`);`)
  await db.run(sql`ALTER TABLE \`settings\` DROP COLUMN \`breadcrumb_home_style\`;`)
}
