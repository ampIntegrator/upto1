...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`pages_blocks_form\` (
  	...
  	\`form_id\` integer,
  	\`framed\` integer DEFAULT true,
  	\`show_heading\` integer DEFAULT true,
  	\`block_name\` text,
  ...
  await db.run(sql`CREATE INDEX \`pages_blocks_form_form_idx\` ON \`pages_blocks_form\` (\`form_id\`);`)
  await db.run(sql`CREATE TABLE \`sections_blocks_form\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`sections_blocks_form_order_idx\` ON \`sections_blocks_form\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`sections_blocks_form_parent_id_idx\` ON \`sections_blocks_form\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`sections_blocks_form_path_idx\` ON \`sections_blocks_form\` (\`_path\`);`)
  await db.run(sql`CREATE INDEX \`sections_blocks_form_form_idx\` ON \`sections_blocks_form\` (\`form_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`forms_blocks_date_locales\` (
  	\`label\` text,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`_locale\` text NOT NULL,
  	\`_parent_id\` text NOT NULL,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`forms_blocks_date\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`forms_blocks_date_locales_locale_parent_id_unique\` ON \`forms_blocks_date_locales\` (\`_locale\`,\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`forms_blocks_radio_options_locales\` (
  	\`label\` text NOT NULL,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`_locale\` text NOT NULL,
  	\`_parent_id\` text NOT NULL,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`forms_blocks_radio_options\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`forms_blocks_radio_options_locales_locale_parent_id_unique\` ON \`forms_blocks_radio_options_locales\` (\`_locale\`,\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`forms_blocks_radio_locales\` (
  	\`label\` text,
  	\`default_value\` text,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`_locale\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`forms_blocks_radio_locales_locale_parent_id_unique\` ON \`forms_blocks_radio_locales\` (\`_locale\`,\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`forms_blocks_tel\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`forms_blocks_tel_order_idx\` ON \`forms_blocks_tel\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`forms_blocks_tel_parent_id_idx\` ON \`forms_blocks_tel\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`forms_blocks_tel_path_idx\` ON \`forms_blocks_tel\` (\`_path\`);`)
  await db.run(sql`CREATE TABLE \`forms_blocks_tel_locales\` (
  	\`label\` text,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`_locale\` text NOT NULL,
  	\`_parent_id\` text NOT NULL,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`forms_blocks_tel\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`forms_blocks_tel_locales_locale_parent_id_unique\` ON \`forms_blocks_tel_locales\` (\`_locale\`,\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`forms_blocks_consent\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`name\` text DEFAULT 'consentement' NOT NULL,
  	\`privacy_href\` text,
  	\`block_name\` text,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`forms\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`forms_blocks_consent_order_idx\` ON \`forms_blocks_consent\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`forms_blocks_consent_parent_id_idx\` ON \`forms_blocks_consent\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`forms_blocks_consent_path_idx\` ON \`forms_blocks_consent\` (\`_path\`);`)
  await db.run(sql`CREATE TABLE \`forms_blocks_consent_locales\` (
  	\`label\` text DEFAULT 'J’accepte d’être recontacté.' NOT NULL,
  	\`privacy_label\` text,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`_locale\` text NOT NULL,
  	\`_parent_id\` text NOT NULL,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`forms_blocks_consent\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`forms_blocks_consent_locales_locale_parent_id_unique\` ON \`forms_blocks_consent_locales\` (\`_locale\`,\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`forms_blocks_step_break\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`_path\` text NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`forms_blocks_step_break_order_idx\` ON \`forms_blocks_step_break\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`forms_blocks_step_break_parent_id_idx\` ON \`forms_blocks_step_break\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`forms_blocks_step_break_path_idx\` ON \`forms_blocks_step_break\` (\`_path\`);`)
  await db.run(sql`CREATE TABLE \`forms_blocks_step_break_locales\` (
  	\`title\` text,
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`_locale\` text NOT NULL,
  	\`_parent_id\` text NOT NULL,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`forms_blocks_step_break\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE UNIQUE INDEX \`forms_blocks_step_break_locales_locale_parent_id_unique\` ON \`forms_blocks_step_break_locales\` (\`_locale\`,\`_parent_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`sections_blocks_form\`;`)
  ...
  await db.run(sql`DROP TABLE \`forms_blocks_date_locales\`;`)
  await db.run(sql`DROP TABLE \`forms_blocks_radio_options\`;`)
  await db.run(sql`DROP TABLE \`forms_blocks_radio_options_locales\`;`)
  await db.run(sql`DROP TABLE \`forms_blocks_radio\`;`)
  await db.run(sql`DROP TABLE \`forms_blocks_radio_locales\`;`)
  await db.run(sql`DROP TABLE \`forms_blocks_tel\`;`)
  await db.run(sql`DROP TABLE \`forms_blocks_tel_locales\`;`)
  await db.run(sql`DROP TABLE \`forms_blocks_consent\`;`)
  await db.run(sql`DROP TABLE \`forms_blocks_consent_locales\`;`)
  await db.run(sql`DROP TABLE \`forms_blocks_step_break\`;`)
  await db.run(sql`DROP TABLE \`forms_blocks_step_break_locales\`;`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "pages_id", "sections_id", "posts_id", "categories_id", "authors_id", "case_studies_id", "case_categories_id", "media_id", "users_id") SELECT "id", "order", "parent_id", "path", "pages_id", "sections_id", "posts_id", "categories_id", "authors_id", "case_studies_id", "case_categories_id", "media_id", "users_id" FROM \`payload_locked_documents_rels\`;`)
