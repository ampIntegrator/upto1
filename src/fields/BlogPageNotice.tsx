...
/**
 * BlogPageNotice — shown at the top of a page's tabs when that page is the blog page (Site
 * settings › Blog): its page top and sections are not displayed, the site shows the list of posts.
 */
...
export function BlogPageNotice() {
  ...
  useEffect(() => {
    ...
    fetch('/api/globals/settings?depth=0', {credentials: 'include'})
      ...
      .then((d: {blog?: {page?: number | string | null}} | null) => setIsBlog(d?.blog?.page != null && String(d.blog.page) === String(id)))
      .catch(() => undefined);
  }, [id]);
