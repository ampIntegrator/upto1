...
/**
 * The figure blocks of a post, declared as column blocks of the section builder (fresh configs from
 ...
 * Hidden from the column picker since 24 Sept. 2026 (Nicolas: figures belong to posts and case
 * studies only); kept in the schema so that no migration drops their tables.
 */
...
export const FIGURE_BLOCKS: ContentBlock[] = [
  {block: keyPointsBlock(), minSpan: minSpan({type: 'keyPoints'}), fill: true, hidden: true},
  {block: ctaBandBlock(), minSpan: minSpan({type: 'ctaBand'}), hidden: true},
  {block: withStatsCapacity(statsBandBlock()), minSpan: minSpan({type: 'statsBand', count: 2}), hidden: true},
  {block: quoteCardBlock(), minSpan: minSpan({type: 'quoteCard'}), maxSpan: 9, fill: true, hidden: true},
  {block: galleryBlock(), minSpan: minSpan({type: 'gallery'}), hidden: true},
];
