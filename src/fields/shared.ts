...
export function titleField(overrides: Partial<TextareaField> & {name: string}): TextareaField {
  return {
    ...
    admin: {
      rows: 3,
      description: 'Un retour à la ligne = une nouvelle ligne du titre. Entourez la partie en serif de <span>…</span>.',
      ...overrides.admin,
    },
    ...overrides,
  } as TextareaField;
...
export function linkGroup(name: string, label: string, opts: {icon?: boolean; required?: boolean} = {}): GroupField {
  const fields: Field[] = [
...
export const slugField: Field = {
  ...
  unique: true,
  index: true,
  admin: {position: 'sidebar', description: 'Minuscules, chiffres et tirets. « accueil » = page d\'accueil.'},
  validate: (value: unknown) => (typeof value === 'string' && /^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(value)) || 'Minuscules, chiffres et tirets uniquement.',
};
