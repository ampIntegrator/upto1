...
## 1. Decisions to confirm with Nicolas (recommendation first)

1. **Sectors**: a `sectors` collection (« Secteurs », title + slug) in the Réalisations group,
   the case studies' taxonomy, filter chips and archives, like the blog's categories. Not the
   blog categories (different vocabulary: Rénovation, Gros œuvre, Couverture…).
2. **Modules chips of the fact sheet** (« Modules Orbita »): a `hasMany` relationship to
   `pages` (the product pages), the chip shows the page title and links to it. Alternative: a
   free text array, no link.
3. **Sheet CTA** (« Réserver une démo »): label and link in the settings global, with an
   optional per-case override (label + link). Alternative: global only.
4. **Key results**: one array « Résultats clés » (value + label, 2 to 4) on the case. The first
   two feed the sheet's mini results; the card's short result (« −68 % délai ») is its own text
   field, falling back to the first key result's value. The stats band in the story stays an
   inserted figure (the author chooses where it goes, as in the blog).
5. **Related case studies**: automatic, three of the same sector then the newest others, exactly
   like the blog's related posts; the same optional « Sections après la réalisation » tab.
6. **Case study page top**: breadcrumb band on paper under the header, then the full-bleed hero
   with the night veil (mockup 23), no caption on the hero (the mockup's « img · à la une » is a
   note, not content). The cover keeps an `alt` only.
7. **14 px minimum**: the sheet's labels (mockup 10 px mono) and the mini results' labels
   (9.5 px) go to 14 px, like the stats band. The card date and chips are still at 12 px from
   before the blog: fix them to 14 px in the same pass (Card, Chip) or leave them, Nicolas's call.
8. **Naming**: collection `case-studies` (tables `case_studies`), global `portfolio`
   (« Réglages des réalisations »), taxonomy `sectors`, archive segment `secteur`
   (/<réalisations>/secteur/<slug>). UI texts in French, code in English.
...
1. **One listing config type.** `src/lib/blog.ts` becomes `src/lib/listings.ts`: a
   ...
   labels, termSegment: 'categorie' | 'secteur'}) built by `listingConfig(global, kind)`.
...
2. **One card conversion.** `postCard` exists twice (`src/lib/posts.ts` and `src/lib/sections.ts`,
   ...
   sector with tone `high`, `result`, `client: {name, location}`, cta « Voir l’étude »).
3. **Archive and related components stop forcing the preset.** `PostArchive` and
   ...
   nav (« Catégories » / « Secteurs ») and its `empty` text from the settings. Keep the component
...
4. **Routes become dispatchers.** `[slug]/[post]` → `[slug]/[entry]/page.tsx`: if `slug` is the
   ...
   the matching listing's `termSegment` (`categorie` for the blog, `secteur` for the cases),
   else 404. `[slug]/page.tsx` and its metadata dispatch on `blog.pageId` or `cases.pageId`
...
5. **Section builder: a second source.** `collectionBlock`'s `source` select gains
   « Dernières réalisations » (`cases`) with `casesLimit`, `casesSector`, `casesCta` (same
...
title, columns title · sector · client · publishedAt, `defaultSort -publishedAt`,
...
- Tab « Fiche projet »: group `sheet`: `client` (text, required), `clientUrl` (text, the globe
  icon link), `location` (text, « Nantes (44) »), `deployment` (text, localized, « 6 semaines ·
  mars 2025 »), `modules` (relationship pages hasMany, decision 2), `results` (array 2–4 of
  {value, label}, decision 4), `cardResult` (text, « −68 % délai », decision 4), `cta` (group
  label + href, optional override, decision 3).
- Tab « Sections après la réalisation »: `sections.field` (optional, as posts).
- Sidebar: `slugField`, `sector` (relationship `sectors`, required), `publishedAt` (date,
  dayOnly). No author.

Rendering (`CasePage.tsx`): `SitePage` tone light, `currentHref` = the cases listing;
breadcrumb band (Réalisations › sector › title) on paper under the header; `CaseHero`;
...
- **`CaseHero`** (`src/components/CaseHero.tsx` + module CSS): full-bleed cover (`next/image`
  ...
  (« Étude de cas » tone `cat`, sector tone `high`), `Heading level 1 type display-2` with
...
- **`CaseSheet`** (`src/components/CaseSheet.tsx`): the fact sheet, a `dl` of rows (label 14 px
  mono uppercase in editorial, value 14 px semibold; rows separated by 1 px lines), the client row
  with an optional globe icon link, the modules row as a wrap of `Chip` tone `accent`, then the
  mini results (two `Stat` size `bar` in silo colour with 14 px labels, mockup 26 px numbers →
  use the Stat's bar size or add a `sheet` size at 26 px), then the split CTA (`Button` with
  `arrow`, full width). Paper card with border, `--shadow`. Sticky like `PostToc`
  (`top: calc(var(--site-header-bar-scrolled, 60px) + 30px)`), not collapsible below 1024 px
  (it goes above the story). Props: `{rows: {label, value, href?}[], modules?: {label,
  href?}[], results?: {value, label}[], cta?: {label, href}, labels}`.
- **`Card` preset `realisation`** exists (chip + result, title, client · location with the pin,
  ...
  55 % white per mockup 24) and the 14 px rule (result 12 px mono → 14, client 13 px → 14).
...
Secteurs, Réglages des réalisations. The site settings stay untouched (Nicolas wants everything
of a listing in its own group, as done for the blog on 17 Sept.).

- `sectors`: title (localized), slug. Same file as `Categories.ts`.
- `case-studies`: § 3.
- Global `portfolio` (« Réglages des réalisations »), tabs like the blog: « Page des
  ...
  « Toutes », readMore « Voir l’étude », termPrefix « Secteur », more « Voir toutes les
  réalisations », relatedEyebrow, relatedTitle, badge « Étude de cas », sheet labels client /
  sector / location / deployment / modules / results, cta label + href, empty text).
...
- Migrations, each reviewed and applied after a backup: `add_sectors_case_studies`,
...
| URL | Content |
|---|---|
| `/<réalisations>` | the chosen page: sector chips, realisation cards in four columns, pagination |
| `/<réalisations>/<slug>` | the case study |
| `/<réalisations>/secteur/<sector>` | archive of a sector, automatic h1 (prefix « Secteur »), no lead |
...
- `scripts/smoke-cases.ts` (`pnpm smoke:cases`): throwaway sector, page and case study with every
  sheet field and figures, chosen as the cases page for the test, settings restored in `finally`;
  checks the list, the case page (hero, sheet rows, mini results, CTA, related), the sector
...
- `seed:demo`: the « Vasseur Construction » case study of mockup 23 (sector Rénovation, the
  four results, the story with the stats band, gallery, key points, quote card); never touches
  the settings.
...
1. `payload` then `astryx` as needed, § 2 generalisation first (steps 1–4, 6, 7, 9), blog smoke
   ...
   studies ».
2. `astryx`: CaseHero, CaseSheet, Card realisation on night and 14 px, showcases, demo pages,
   registry + tests.
3. `payload`: sectors, case studies, portfolio global (migrations), case page and list, case card
