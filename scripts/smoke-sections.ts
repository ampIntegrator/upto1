...
async function main() {
  ...
  const expectError = async (label: string, rows: ReturnType<typeof column>[][], pattern: RegExp) => {
    try {
      const accepted = await payload.create({collection: 'pages', data: {title: 'Smoke (invalid)', slug: `${slug}-invalid`, hero, sections: [section(rows)]} as never});
      await payload.delete({collection: 'pages', id: accepted.id});
  ...
  await expectError('carousel of 25 latest posts (24 at most)', [[column(12, {blockType: 'collection', layout: 'carousel', perView: '3', source: 'posts', postsLimit: 25})]], /24/);
  await expectError('custom « see all » button without a link', [[column(12, {blockType: 'collection', layout: 'carousel', perView: '3', source: 'posts', moreLink: 'custom', moreLabel: 'Sans lien'})]], /libellé et un lien|label and a link/);
  ...
  // the post figures are hidden from the column picker (24 Sept. 2026): a column cannot take one anew
  for (const figure of [
    {blockType: 'keyPoints', eyebrow: 'À retenir', content: {root: {type: 'root', children: [{type: 'paragraph', children: [{type: 'text', text: 'Point', format: 0}]}]}}},
    {blockType: 'quoteCard', quote: 'Citation.', name: 'Témoin'},
    {blockType: 'statsBand', items: [{value: '1', label: 'L1'}, {value: '2', label: 'L2'}]},
    {blockType: 'ctaBand', variant: 'arrow', title: 'Bandeau', button: {label: 'Aller', href: '#'}},
    {blockType: 'gallery', images: []},
  ]) await expectError(`post figure « ${figure.blockType} » in a column`, [[column(6, figure), column(6)]], /plus proposé|no longer offered|pas autorisé|not allowed/);
